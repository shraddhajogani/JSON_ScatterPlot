 ...
 */
  List<PositionData> chartData = [];
Future<String> _loadPositionDataAsset() async {
  return await rootBundle.loadString('asset/data.json');
}

Future loadPositionData() async {
  String jsonString = await _loadPositionDataAsset();
  final jsonResponse = json.decode(jsonString);
  setState(() {
    for (Map i in jsonResponse) {
      chartData.add(PositionData.fromJson(i));
    }
  });
}
        ...
        child: Center(
          child: Column(
            children: <Widget>[
              Padding(
                child: Center(
                    child: Text('3D ScatterPlot', style: TextStyle(fontSize: 20))),
                          ...
                          series: [{
                              ...
                              data: [[0,0,5],[0,1,1],[0,2,0],[0,3,0],[0,4,0],[0,5,0],[0,6,0],
                                ...
                                [6,18,0],[6,19,0],[6,20,1],[6,21,2],[6,22,2],[6,23,6]]
                                  .map(function (item) {
                                  return {
                                      xcoordinate: [item[1], item[0], item[2]],
                                  }
                              }),
                              shading: 'lambert',
                              label: {
...
class PositionData {
  PositionData(this.xcoordinate, this.ycoordinate);

  final String xcoordinate;
  final int ycoordinate;

  factory PositionData.fromJson(Map<String, dynamic> parsedJson) {
    return PositionData(
      parsedJson['xcoordinate'].toString(),
      parsedJson['ycoordinate'] as int,
    );
  }
}